local SUDO = 353171541 -- put Your ID here! <===
...
----------------------------------------
local function delmsg (MaTaDoR,RADICAL)
    msgs = MaTaDoR.msgs 
    for k,v in pairs(RADICAL.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(RADICAL.messages_[0].chat_id_,{[0] = RADICAL.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(RADICAL.messages_[0].chat_id_, RADICAL.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
----------------------------------------
...
if ((matches[1]:lower() == 'rmsg' and not lang) or (matches[1] == 'پاکسازی' and lang)) and is_mod(msg) then
    if redis:get("delchat:"..msg.to.id) and not is_admin(msg) then
    return "`این کار فقط هر 1ساعت 1بار ممکن میباشد لطفا بعد از یک ساعت دوباره امتحان کنید`"
    end
...
if not lang then
   tdcli.sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './file/ping.gif', 'Online..!', dl_cb, nil)
   else
   tdcli.sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './file/ping.gif', 'انلاین..!', dl_cb, nil)
   end
end
--------------------- Welcome -----------------------
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
	...
	if ((matches[1]:lower() == "sendfile" and not lang) or (matches[1] == 'ارسال فایل' and lang)) and matches[2] and matches[3] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, send_file, '@RADICALBOTTEAM', dl_cb, nil)
	end
	if ((matches[1]:lower() == "sendplug" and not lang) or (matches[1] == 'ارسال پلاگین' and lang)) and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, plug, '@RADICALBOTTEAM', dl_cb, nil)
    end
...
return tdcli.sendMessage(msg.to.id, msg.id, 0, "*کاربر "..matches[2].." از محرومیت گروه های ربات خارج شد*", 0, "md")
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   ...
   if msg.to.type ~= 'pv' then
 if ((matches[1]:lower() == "ban" and not lang) or (matches[1] == "بن" and lang)) and is_mod(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if matches[1]:lower() == 'matador' or matches[1] == 'رادیکال' then
...
✳️برای گروه های 0 تا 500 نفر ‌
➰1 ماهه  5 هزار تومان
➰3 ماهه 12 هزار تومان

〰〰〰〰〰〰〰
✳️برای گروه های 500 تا 1000 نفر 
➰1 ماهه  6 هزار تومان
➰3 ماهه  14 هزار تومان

〰〰〰〰〰〰〰
✳️برای گروه های 1000 تا 3000 نفر ماهانه 
➰1 ماهه  8 هزار تومان 
➰3 ماهه  20 هزار تومان

〰〰〰〰〰〰〰
✳️برای گروه های 3000 تا 5000  نفر ماهانه 
